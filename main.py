...
output_movie = cv2.VideoWriter("deteccion1.avi", fourcc, fps, (width, height))
bird_movie = cv2.VideoWriter(
    "visi1.avi", fourcc, fps, (int(width * scale_w), int(height * scale_h))
)
...
size1 = 100
size2 =150
logo = cv2.resize(logo, (size2, size1))
...
while cap.isOpened():
    ...
    roi = frame[-size1-10:-10, -size2-10:-10]
    ...
    if not ret:
        print("Final del procesamiento, TD 2021")
        break
    ...
    last_h = 75
    text = "ALARMAS < 1.5MTS: " + str(int(total_six_feet_violations))
  ...
  #  pedestrian_detect, last_h = put_text(pedestrian_detect, text, text_offset_y=last_h)
    ...
    cv2.imshow("CCTVAL 2021", pedestrian_detect)
    cv2.waitKey(1)
